...
class AddTwoNumbers {
    ...
    static void addTwoLists(AddTwoNumbers first, AddTwoNumbers second, AddTwoNumbers res, GfG g) {
	    if (first.head == null) { 
...
class GfG {
    ...
    Node temp;
    void addCarryToRemaining(Node head, AddTwoNumbers res)  { 
        // Write code here
        if(head!=cur){
            addCarryToRemaining(head.next, res);
            int curr_sum = head.data+carry;
            res.push(curr_sum%10);
            carry = curr_sum/10;
        }
    } 
    
	void addSameSize(Node head1, Node head2, AddTwoNumbers res) { 
	    // Write code here
        if(head1 == null)
            return;
